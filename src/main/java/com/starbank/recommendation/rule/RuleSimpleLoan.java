...
public class RuleSimpleLoan extends TemplateForRules {
    ...
    private final com.starbank.recommendation.repository.jdbc.H2Repository repository;
    ...
    protected boolean isEligible(UUID user_id) {
        boolean hasCredit = repository.usesProductType(user_id, ProductType.CREDIT);
        long sumDepositDebit = repository.sumByTransactionTypeAndProductType(user_id,
                TransactionType.DEPOSIT,
                ProductType.DEBIT);
        long sumWithdrawDebit = repository.sumByTransactionTypeAndProductType(user_id,
